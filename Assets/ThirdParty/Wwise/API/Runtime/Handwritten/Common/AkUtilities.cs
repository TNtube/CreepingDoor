...
public partial class AkUtilities
{
	...
	{
        if (string.Equals(
            Path.GetFullPath(sourceDirName).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar),
            Path.GetFullPath(destDirName).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar),
            System.StringComparison.OrdinalIgnoreCase)
        ) {
            UnityEngine.Debug.LogError("WwiseUnity: Cannot DirectoryCopy with same source & target");
            return false;
        }
...
public partial class AkUtilities
{
	...
	{
		const string AssetWwisePathParent = "Assets/ThirdParty/Wwise/API/";
