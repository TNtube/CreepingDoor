...
namespace CodeStage.AdvancedFPSCounter.CountersData
{
	...
	public class FPSCounterData : UpdatableCounterData
	{
		...
		private const string RenderTextStart = ColorTextStart;
        ...
        private int currentRenderAverageSamples;
        private float currentRenderAverageRaw;
        ...
        /// <summary>
		/// Last calculated Render Min value.
		/// \sa Render
		/// </summary>
		public float LastRenderMin { get; private set; } = -1;

        /// <summary>
		/// Last calculated Render Max value.
		/// \sa Render
		/// </summary>
		public float LastRenderMax { get; private set; } = -1;
		...
		public void ResetAverage()
		{
            ...
            currentRenderAverageSamples = 0;
            currentRenderAverageRaw = LastRenderValue;
		}
		...
		internal override void UpdateValue(bool force)
		{
			...
			if (render)
			{
				if (renderTimeBank > 0)
				{
					...
					if (Math.Abs(renderTime - LastRenderValue) > 0.0001f || force)
					{
                        ...
                        // CUSTOM CODE
                        // Calculate Render Min-Max values
                        if (minMaxIntervalsSkipped > minMaxIntervalsToSkip) {
                            if (renderTime > LastRenderMin || LastRenderMin == -1) LastRenderMin = renderTime;
                            if (renderTime < LastRenderMax || LastRenderMax == -1) LastRenderMax = renderTime;

                            // CUSTOM CODE
                            // Calculate Render Average values 
                            currentRenderAverageSamples++;
                            if (currentRenderAverageSamples > averageSamples && averageSamples != 0)
                                currentRenderAverageRaw += (renderTime - currentRenderAverageRaw) / (averageSamples + 1);
                            else currentRenderAverageRaw += (renderTime - currentRenderAverageRaw) / currentRenderAverageSamples;
                        }
			...
			if (dirty && main.OperationMode == OperationMode.Normal)
			{
				...
				if (realtimeFPS)
				{
                    ...
                    if (render) {
                        text.Append("  ")
                            .Append(colorCachedRender).Append("REN : ")
				...
				if (average)
				{
                    ...
                    if (render) {
                        text.Append("  ")
                           .Append(colorCachedRender).Append("AVG : ")
                           .AppendLookup(currentRenderAverageRaw).Append(" MS")
                           .Append(ColorTextEnd);
                    }
				}
				...
				if (minMax)
				{
                    ...
                    if (render && minMaxTwoLines) {
                        text.Append("  ")
                            .Append(colorCachedRender).Append("MIN : ")
                            .AppendLookup(LastRenderMin).Append(" MS")
                            .Append(ColorTextEnd);
                    }
                    ...
                    if (render && minMaxTwoLines) {
                        text.Append("  ")
                            .Append(colorCachedRender).Append("MAX : ")
                            .AppendLookup(LastRenderMax).Append(" MS")
                            .Append(ColorTextEnd);
                    }
                }
				...
				/*if (render)
				{
                    ...
                    text.Append(renderNewLine ? AFPSCounter.NewLine : AFPSCounter.Space)
                        .Append("AVG: ").AppendLookup(currentRenderAverageRaw).Append(" MS");
                        
                    text.Append(renderNewLine ? AFPSCounter.NewLine : AFPSCounter.Space)
                        .Append("MIN: ").AppendLookup(LastRenderMin).Append(" MS");

                    text.Append(renderNewLine ? AFPSCounter.NewLine : AFPSCounter.Space)
                        .Append("MAX: ").AppendLookup(LastRenderMax).Append(" MS");
