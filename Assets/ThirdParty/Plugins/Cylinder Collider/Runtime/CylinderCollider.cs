...
namespace cylinder_collider
{
    ...
    public class CylinderCollider : MonoBehaviour
    {
        ...
        public float pipeThickness
        {
            ...
            set
            {
                _pipeThickness = Mathf.Clamp01(value);
                UpdateSize();
            }
        }
        ...
        {
            get => colliders != null && colliders.Count != 0 ? colliders[0].hasModifiableContacts : false;
            set
            {
                foreach (var col in colliders)
                {
        ...
        /// CylinderCollider is composed of multiple child colliders
        ...
        public void Validate()
        {
            ...
            if (shape == Shape.Pipe) _pipeThickness = Mathf.Clamp01(_pipeThickness);
            CreateRoot();
            UpdateResolution();
            UpdateIsTrigger();
            UpdateCenter();
            ...
            else
            {
                for (int i = 0; i < colliders.Count; i++)
                {
                    if(!colliders[i].TryGetComponent(out PhysicsListener listener))
                    {
                        ...
                        listener.type = PhysicsListener.Type.CylinderCollider;
                        listener.cylinderCollider = this;
        ...
        void CreateRoot()
        {
            colliderRoot = transform.Find("Cylinder Collider Root");
            if (colliderRoot == null)
            {
                colliderRoot = new GameObject("Cylinder Collider Root").transform;
        ...
        void CalcBounds()
        {
            ...
            _center = b.center;
            var localSize = b.size;
            _radius = Mathf.Max(localSize.x, localSize.z) / 2;
            _height = localSize.y; 
        }
        ...
        void UpdateResolution()
        {
            ...
            if (diff > 0) // should add
            {
                ...
                {
                    var col = new GameObject("col").AddComponent<BoxCollider>();
            ...
            else // should remove
            {
                ...
                {
                    var col = colliderRoot.GetChild(colliderRoot.childCount - 1).GetComponent<BoxCollider>();
            ...
            {
                colliders.Add(colliderRoot.GetChild(i).GetComponent<BoxCollider>());
            }

            UpdateSize();
        }

        void UpdateSize(BoxCollider col, int index)
        {
            ...
            switch (shape)
            {
                case Shape.Cylinder:
                {
                    var x = 2f * radius * Mathf.Sin(Mathf.PI / polyCount);
                    var y = height;
                    var z = Mathf.Sqrt(Mathf.Pow(2 * radius, 2) - Mathf.Pow(x, 2));
                ...
                case Shape.Pipe:
                {
                    var x = 2f * radius * Mathf.Sin(Mathf.PI / polyCount);
                    var y = height;
                    var z = Mathf.Lerp(0, Mathf.Sqrt(Mathf.Pow(2 * radius, 2) - Mathf.Pow(x, 2)) * 0.5f,
                        _pipeThickness);

                    var distFromCenter = Mathf.Sqrt(Mathf.Pow(radius, 2) - Mathf.Pow(x * 0.5f, 2)) - z * 0.5f;


                    col.center = new Vector3(0, 0, distFromCenter);
                    col.size = new Vector3(x, y, z);
                    col.transform.localRotation = Quaternion.Euler(0, index * (360f / polyCount), 0);
                    break;
                }
...
#if UNITY_6000_0_OR_NEWER
        public BoxGeometry[] GetGeometry()
        {
            var array = new BoxGeometry[colliders.Count];
            for (int i = 0; i < colliders.Count; i++)
            {
                var c = colliders[i];
                var box = c.GetGeometry<BoxGeometry>();
                array[i] = box;
            }
            return array;
        }
        
        public NativeArray<BoxGeometry> GetGeometry(Allocator allocator)
        {
            var array = new NativeArray<BoxGeometry>(colliders.Count, allocator);
            for (int i = 0; i < colliders.Count; i++)
            {
                var c = colliders[i];
                var box = c.GetGeometry<BoxGeometry>();
